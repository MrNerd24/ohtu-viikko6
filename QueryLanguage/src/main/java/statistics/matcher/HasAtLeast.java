...
public class HasAtLeast implements Matcher {
    ...
    public HasAtLeast(Matcher matcher, int i, String category) {
        this(i, category);
    ...
    public boolean matches(Player p) {
        if (matcher != null) {
            return matcher.matches(p) && CheckCondition(p);
        } else {
            return CheckCondition(p);
        }
    }    

    private boolean CheckCondition(Player p) throws IllegalStateException {
        try {
